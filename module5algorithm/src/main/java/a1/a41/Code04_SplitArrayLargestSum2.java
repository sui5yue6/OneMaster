...
public class Code04_SplitArrayLargestSum2 {
...
//                for (int k = i; k >=j-1; k--) {
//                    dp[i][j] = Math.min(dp[i][j],Math.max(dp[k-1][j-1],))
//                }
//            }
//        }
//    }

    public static int f2(int[] arr, int M) {
        int N = arr.length;
        int[] sum = new int[arr.length + 1];
        for (int i = 0; i < arr.length; i++) {
            sum[i + 1] = sum[i] + arr[i];
